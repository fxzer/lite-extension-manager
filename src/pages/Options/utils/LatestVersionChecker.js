...
const check = async () => {
  ...
  const apiUrl = "https://api.github.com/repos/fxzer/lite-extension-manager/releases/latest"
...
export const compareVersion = async (currentVersion) => {
  ...
  if (current > latest) {
    console.warn("Current version is newer than cached latest version", current, "  ", latest)
    return null
  }
